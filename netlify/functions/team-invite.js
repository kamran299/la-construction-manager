...
export default async (request) => {
  ...
  if (!companyId || !fullName || !email || !allowedRoles.includes(role)) {
  ...
  const memberResponse = await fetch(`${url}/rest/v1/company_members?on_conflict=company_id,user_id`, {
    ...
    body: JSON.stringify({ company_id: companyId, user_id: invited.id, full_name: fullName, email, role, is_active: true }),
  });
