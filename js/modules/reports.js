...
export function createReportsModule({ supabase, session, companyId, membership, canManage }) {
  ...
  async function callAi(payload, { retryGatewayTimeout = false } = {}) {
    const attempts = retryGatewayTimeout ? 2 : 1;
    for (let attempt = 1; attempt <= attempts; attempt += 1) {
      ...
      try { data = JSON.parse(responseText); }
        ...
        if (response.status === 504 && attempt < attempts) {
          message.textContent = "The AI server took too long. Trying once more automatically...";
          continue;
        }
        if (response.status === 504) throw new Error("The AI server took too long twice. Please wait one minute and try again.");
        throw new Error(`AI server returned an unreadable response (HTTP ${response.status}).`);
      }
