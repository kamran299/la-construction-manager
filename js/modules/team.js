...
export function createTeamModule({ supabase, session, companyId, canManage, managerRole }) {
  ...
  async function load() {
    ...
    const accounts = (memberResult.data || []).filter((member) => member.is_active && !member.login_alias_of).map((member) => {
      const isSelf = member.user_id === session.user.id;
      const canEditMember = canManage && !isSelf && member.role !== "owner_admin" && (managerRole === "owner_admin" || member.role !== "project_manager");
      const canSetOwnPhone = isSelf;
      const contact = [member.email, member.phone].filter(Boolean).join(" · ") || "No contact details";
      const editor = canEditMember ? `<form class="team-member-editor" data-member-editor="${escapeHtml(member.id)}" hidden><label>Full name<input name="fullName" value="${escapeHtml(member.full_name || "")}" required></label><label>Mobile phone<input name="phone" type="tel" autocomplete="tel" value="${escapeHtml(member.phone || "")}" placeholder="(650) 555-0123"></label><label>Role<select name="role">${Object.entries(ROLES).filter(([value]) => value !== "owner_admin" && (managerRole === "owner_admin" || value !== "project_manager")).map(([value, label]) => `<option value="${value}" ${member.role === value ? "selected" : ""}>${label}</option>`).join("")}</select></label><div class="team-editor-actions"><button class="primary-button" type="submit">Save changes</button><button class="danger-outline-button" type="button" data-remove-member="${escapeHtml(member.id)}">Remove access</button></div></form>` : canSetOwnPhone ? `<form class="team-member-editor" data-member-editor="${escapeHtml(member.id)}" data-self-phone="true" hidden><label>Mobile phone for text-code sign-in<input name="phone" type="tel" autocomplete="tel" value="${escapeHtml(member.phone || "")}" placeholder="(650) 555-0123" required></label><div class="team-editor-actions"><button class="primary-button" type="submit">Save phone login</button></div></form>` : "";
      const action = canEditMember || canSetOwnPhone ? `<button class="secondary-button" type="button" data-edit-member="${escapeHtml(member.id)}">${canSetOwnPhone ? "Set up text login" : "Edit"}</button>` : '<i class="member-status is-active">Active</i>';
      return `<article class="workspace-card team-member" data-member-row="${escapeHtml(member.id)}"><span class="avatar">${escapeHtml((member.full_name || member.email || "U")[0].toUpperCase())}</span><div><strong>${escapeHtml(member.full_name || "User")}</strong><small>${escapeHtml(contact)}</small></div><span class="role-badge">${escapeHtml(ROLES[member.role] || member.role)}</span>${action}${editor}</article>`;
    }).join("");
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      const response = await fetch("/.netlify/functions/team-invite", { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${session.access_token}` }, body: JSON.stringify({ companyId, fullName: document.querySelector("#inviteName").value.trim(), email: document.querySelector("#inviteEmail").value.trim(), phone: document.querySelector("#invitePhone").value.trim(), role: document.querySelector("#inviteRole").value, siteUrl: location.origin }) });
  ...
  list.addEventListener("submit", async (event) => {
    ...
    const fields = targetType === "member"
      ? { fullName: formData.get("fullName"), phone: formData.get("phone"), role: formData.get("role"), selfPhoneOnly: memberEditor?.dataset.selfPhone === "true" }
